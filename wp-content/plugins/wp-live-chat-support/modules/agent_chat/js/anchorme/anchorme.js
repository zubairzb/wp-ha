...
function defaultOptions(options) {
    ...
    if (typeof options.customTlds !== "object")
        { options.customTlds = []; }
    if (typeof options.displayText !== "string" && typeof options.displayText !== "function")
        { options.displayText = ""; }
    return options;
}
...
function default_1(str, customTlds) {
    if (customTlds === void 0) { customTlds = []; }
    ...
    if (typeof match[3] !== "string")
        { return false; }
    var tldsToCheck = customTlds.length > 0 ? lists.tlds.concat(customTlds) : lists.tlds;
    if (tldsToCheck.indexOf(match[3].toLowerCase()) === -1)
        { return false; }
    // validate port
...
function default_1(inputArr, options) {
    return inputArr.map(function (fragment, index) {
        ...
        // test 2: it's a URL
        if ((!urlObj) && options.urls && url.default(encoded, options.customTlds)) {
            urlObj = {
                reason: "url",
...
function url2tag(fragment, options) {
    ...
    var original = options.displayText !== '' ? options.displayText : fragment.raw;
...
anchorme.validate = {
    ...
    url: function (input, customTlds) {
        ...
        return url.default(input, customTlds);
    },
    email: email.default
};
